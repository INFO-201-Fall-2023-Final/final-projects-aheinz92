             ...
             tabPanel("Stacked Bar Chart",
                      fluidRow(
                        column(4,
                               wellPanel(
                                 style = "padding:20px; background-color:#333; color:#fff; border-radius: 8px;",
                                 sliderInput("barSlider", "Adjust Parameter:", min = 1, max = 100, value = 30),
...
server <- function(input, output) {
  ...
  # Song Info
  output$songInfo <- renderText({
    # Update this with logic to display song information based on selected song once we have list finalized, and if we wont more info at all (don't need)
    paste("Information for the song:", input$songSelector)
  })
  output$stackedBarChart <- renderPlot({
    # Replace with the data/chart code
    ggplot(data = mpg, aes(x = factor(cyl), fill = class)) + 
